...
const main = async () => {
  // -2 is the default value for match field , meanng not verified ye\t
  const query = `
      select * from tripper where Match_field = -2 or meta_data_id = -2; 
    `;
  //default value will be 2 after comapring will be -1 or > 1
  ...
  return new Promise((resolve) => {
    connection.query(query, async (error, results, fields) => {
      ...
      for (let i of results) {
        console.log(i);
        try {
          const google = await compare(i, currentIndex);
          const meta = await compareMetadata(i, currentIndex);
          await updateDB({
            id: google?.id,
            google: google?.nextId,
            meta: meta?.nextId,
          });
...
const calculate = (tripper, gmm_next, gmm_prev, index) => {
  //find nearest field in gmm
  const tripper_time = tripper["Date_time"];
...
const compare = async (tripper_row, index) => {
  const time = tripper_row["Date_time"];
  const tripperPrediction = tripper_row["Primary_image"];
  ...
  return await new Promise((resolve, reject) => {
    try {
      connection.query(queryNextTime, async (error, resultsN, fields) => {
        if (error) throw new Error(error.message);
        connection.query(queryPrevTime, async (error, resultsP, fields) => {
          if (error) throw new Error(error.message);
          ...
          if (nearRow) {
            ...
            const isValid = isMatch(tripperPrediction, googlePrediction);

            //-1 = failure
            resolve({
              id: tripper_row.id,
              nextId: isValid ? nearRow.id : -1,
            });
          } else {
...
const compareMetadata = async (tripper_row, index) => {
  const time = tripper_row["Date_time"];
  const tripperPrediction = tripper_row["Primary_image"];
  const timeString = moment(time).format("YYYY-MM-DD HH:mm:ss");
      ...
      connection.query(queryNextTime, async (error, resultsN, fields) => {
        ...
        connection.query(queryPrevTime, async (error, resultsP, fields) => {
          ...
          if (nearRow) {
            ...
            resolve({ id: tripper_row.id, nextId: isValid ? nearRow.id : -1 });

            //-1 = failure
            // updateDB({
            //   id: tripper_row.id,
            //   gmmKey: isValid ? nearRow.id : -1,
            // });
          } else {
            //this happens when there is no matching timestamp in GMM table (within mentioned margin)
            //console.log("###", index, timeString, "NO MATCH");
